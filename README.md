...
同時に `pc_excel/fax_ledger.xlsx` にも自動追記します。

## 自社PCのExcel台帳へ直接入力する

画面で読み取ったあと、`PCのExcelに入力` を押すと、PC内の台帳へ追記します。追記された行は、最初は必ず `社員確認待ち` になります。

標準の保存先:

```text
pc_excel/fax_ledger.xlsx
```

別のExcel台帳を使いたい場合は、起動時に `LEDGER_PATH` を指定します。

```powershell
$env:LEDGER_PATH="C:\Users\Public\Documents\発注台帳.xlsx"
python app.py 8765
```

Excel台帳を開いたままだと書き込みに失敗する場合があります。その場合はExcelを閉じてからもう一度実行してください。
...
社員の人が内容を確認したら、`確認状況` を `確認済み` に変更し、確認者と確認日時を入れる運用にできます。
